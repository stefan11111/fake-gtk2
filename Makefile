...
all: libgtk-x11-2.0.so.0 libgdk-x11-2.0.so.0

.c.o:
	${CC} ${XCFLAGS} -c -o $@ $<

libgtk-x11-2.0.so.0:
	${CC} ${XCFLAGS} libgtk-x11-2.0.c -o libgtk-x11-2.0.so.0 ${XLDFLAGS},-soname,libgtk-x11-2.0.so.0

libgdk-x11-2.0.so.0:
	${CC} ${XCFLAGS} libgdk-x11-2.0.c -o libgdk-x11-2.0.so.0 ${XLDFLAGS},-soname,libgdk-x11-2.0.so.0

install: libgtk-x11-2.0.so.0 libgdk-x11-2.0.so.0
	mkdir -p ${DESTDIR}/usr/lib64
	cp -f *.so.0 ${DESTDIR}/usr/lib64
	ln -rsf ${DESTDIR}/usr/lib64/libgtk-x11-2.0.so.0 ${DESTDIR}/usr/lib64/libgtk-x11-2.0.so
